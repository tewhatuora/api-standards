...
|User Info<br/>`/userinfo`| API Provider| Access Token| Use an access token to get information about the authenticated health participant<li><ApiStandard id="HNZAS_SHOULD_PROVIDE_USERINFO" type="SHOULD" toolTip="API Providers SHOULD provide a userinfo endpoint.">The API Provider **SHOULD** provide a userinfo endpoint</ApiStandard></li><li><ApiStandard id="HNZAS_MUST_USE_TLS" type="MUST" toolTip="TLS MUST be used during authentication and to provide integrity protection in transit." dupe="true">**MUST** be protected with TLS</ApiStandard></li><li><ApiStandard id="HNZAS_MUST_VALIDATE_OAUTH2_TOKENS" type="MUST" toolTip="Consumers of OAuth 2.0 secured APIs MUST validate Issuer, Signature, Claims and Scopes before using an issued token.">The Access Token used to authenticate to the endpoint **MUST** be validated for validity (time), Issuer and Signature</ApiStandard></li>|
