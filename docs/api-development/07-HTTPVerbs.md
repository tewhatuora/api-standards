...
[API Providers](../api-concepts/ComponentDefinitions#api-producer) and [API Consumers](../api-concepts/ComponentDefinitions#api-consumer) **MUST** design APIs that are PUT tolerant. What this means is that both producers and consumers should be aware that the resource that is `PUT` to the server becomes the most up to date representation of that resource. This exposes the `PUT` method to a commonly found race condition. This condition is explained in the simple sequence below.
