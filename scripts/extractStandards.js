...
// Regular expression to match the desired format (e.g., A-1 or A-2.1)
const idFormatRegex = /^[A-Z]-\d+(\.\d+)?$/;
...
function extractDataFromHTML(filePath, htmlContent) {
  ...
  $('[data-standard-type]').each((index, element) => {
    ...
    if (!idFormatRegex.test(id)) {
      throw new Error(`Invalid ID Standards format found: ${id}. It should be in the format A-1 or A-1.1`);
    }
